...
import { availableChromeDevtoolsTools } from "./lazy-tools.ts";
import { state } from "./runtime.ts";
import { loadSettings, type SettingsLoadResult } from "./settings.ts";
import {
	...
	const menu = defineMenu<MenuSnapshot, MainScreen, MainAction>({
			...
			main: ({ state: current }) => ({
				...
				items: [
					...
					},
					{
						...
						description:
							current.activeTools.length === CHROME_DEVTOOLS_TOOL_NAMES.length
								? `Preview 0 of ${CHROME_DEVTOOLS_TOOL_NAMES.length}; other active tools stay enabled.`
								: `Preview ${CHROME_DEVTOOLS_TOOL_NAMES.length} of ${CHROME_DEVTOOLS_TOOL_NAMES.length}; other active tools stay enabled.`,
						disabled: Boolean(current.mutationBlockedReason),
						disabledReason: current.mutationBlockedReason,
						action: "bulk",
					},
		...
		actions: {
			...
			settings: async ({ state: current }) => {
				const { showChromeDevtoolsBrowserSettings } = await import("./browser-settings-menu.ts");
...
function buildToolReview(current: ToolWorkflowState) {
	const catalog = CHROME_DEVTOOLS_TOOL_NAMES;
