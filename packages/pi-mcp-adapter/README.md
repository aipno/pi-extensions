...
Use MCP servers with [Pi](https://github.com/badlogic/pi-mono/) without burning your context window. One proxy tool (~200 tokens) instead of hundreds of MCP tool definitions. The agent discovers what it needs on-demand; servers only start when you actually use them.

Ported from [`nicobailon/pi-mcp-adapter`](https://github.com/nicobailon/pi-mcp-adapter) v2.28.0 into this repo's package layout.
...
```bash
pi install /path/to/packages/pi-mcp-adapter
# or run once without installing:
pi -e /path/to/packages/pi-mcp-adapter
```

Restart Pi after installation, or `/reload`.

Global drop-in: `~/.pi/agent/extensions/pi-mcp-adapter/` (directory with `index.ts`). Project-local: `.pi/extensions/`.
...
```sh
npm install
npm run build:public  # emit public helper JS under dist/
npm run typecheck     # tsc --noEmit
npm test              # vitest + public export checks
npm run test:oauth    # OAuth flow tests (memory store)
```

The interactive-visualizer example tests expect a built `examples/interactive-visualizer/dist/`:
...
MIT — see [LICENSE](LICENSE). Upstream: [nicobailon/pi-mcp-adapter](https://github.com/nicobailon/pi-mcp-adapter).
