...
import { config as tuiStyleConfig } from "../extensions/config/config.ts";
import {
...
test("compact thinking patches the runtime component with pi-tui config", () => {
...
test("thinking preview counts wrapped hidden lines and does not restyle from cache", () => {
	...
	try {
		...
		const bodyToken = tuiStyleConfig.dimThinkingText ? "dim" : "thinkingText";
		assert.ok(
			!firstPlain.some(
