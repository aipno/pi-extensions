...
test("pi-tui registers the write override at session_start", async () => {
...
test("expanded pi-tui tools use Pi's native background card", async () => {
	const events = new Map<string, Function>();
	const pi = {
		registerCommand() {},
		registerShortcut() {},
...
test("pi-tui is the default renderer and exclusions preserve dedicated renderers", () => {
	const builtIn = {
		name: "edit",
		renderShell: "self",
		renderCall() {},
...
test("Agent keeps its dedicated renderer under pi-tui", async () => {
	const events = new Map<string, Function>();
	const pi = {
		registerCommand() {},
		registerShortcut() {},
...
test("global renderer reload chains external wrappers and shutdown restores them", async () => {
	...
	try {
		...
		const firstPatch = (globalThis as any)[Symbol.for("pi.tui.global-tool-render-patch")];
		...
		delete (globalThis as any)[Symbol.for("pi.tui.global-tool-render-patch")];
	}
});
...
test("global renderer migrates legacy Symbol state without retaining old wrappers", async () => {
	...
	(globalThis as any)[Symbol.for("pi.tui.global-tool-render-patch")] = legacy;
	const ctx = {
		mode: "tui",
		hasUI: true,
		ui: { theme: {}, setStatus() {}, requestRender() {} },
	...
	try {
		...
		await events.get("session_start")?.({}, ctx);
		const migrated = (globalThis as any)[Symbol.for("pi.tui.global-tool-render-patch")];
