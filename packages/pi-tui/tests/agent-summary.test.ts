/**
 * agent-summary 测试：分类 / 去重计数 / failed / minToolCount 门槛 /
 * renderer 产出 `> *Ran 1 command, read 1 file · 5m 12s*` 形态（stripAnsi 后断言）。
 */
import { test } from "node:test";
import assert from "node:assert/strict";
import { initTheme } from "@earendil-works/pi-coding-agent";
import { stripAnsi } from "../extensions/utils/ansi-text.ts";
import { DEFAULT_CONFIG, normalizeConfig } from "../extensions/config/config.ts";
...
test("config 装配：enableAgentSummary 默认开启、opt-out 解析", () => {
	assert.equal(DEFAULT_CONFIG.enableAgentSummary, true);
	assert.equal(normalizeConfig({}).enableAgentSummary, true);
	assert.equal(normalizeConfig({ enableAgentSummary: false }).enableAgentSummary, false);
});
import { config } from "../extensions/config/config.ts";

test("zh/en 双语：summaryLine/summaryMarkdown 输出随 config.language 切换", () => {
	const previous = config.language;
	try {
		const summary = new AgentRunSummary(0);
		summary.recordToolStart("bash", { command: "ls" });
		summary.recordToolStart("read", { path: "a.ts" });
		summary.recordToolStart("grep", {});
		summary.recordToolResult(true);
		const data = summary.snapshot(312_000);

		config.language = "zh";
		assert.equal(
			summaryLine(data),
			"运行了 1 条命令，读取了 1 个文件，使用了 1 个其他工具，1 个失败 · 5m 12s",
		);
		assert.equal(
			summaryMarkdown(data, true),
			"> *运行了 1 条命令，读取了 1 个文件，使用了 1 个其他工具，1 个失败 · 5m 12s*",
		...
		config.language = "en";
		assert.equal(summaryLine(data), "Ran 1 command, read 1 file, 1 other tool, 1 failed · 5m 12s");
	} finally {
