// /context 测试：纯函数（capParts / resolveUsedTokens / collectContextBreakdown /
// embeddedTokens）+ 主对话框渲染（mock registerCommand + ui.custom）+ text-preview 复用契约。
// escCloseHitbox 的契约：
...
import contextUsageExtension from "../extensions/feature/context.ts";
import { escCloseHitbox, hasActiveTextPreview, showTextPreview } from "../extensions/feature/text-preview.ts";
...
test("formatTokens 千分位缩写", () => {
	...
	assert.equal(formatTokens(1_500_000), "1500k");
});
...
/** /context 主对话框的最小 harness：捕获 ui.custom 工厂，首次 render 断言结构。 */
function contextDialogHarness() {
	let registered: { name: string; handler: (args: string, ctx: any) => Promise<void> } | undefined;
	let factory: any;
	let doneFn: ((value?: any) => void) | undefined;
	const ctx = {
		mode: "tui",
		hasUI: true,
		ui: {
		...
		getContextUsage: () => ({ tokens: 30_000, percent: 11, contextWindow: 272_000 }),
		getSystemPrompt: () => "system".repeat(100),
		getSystemPromptOptions: () => ({
	...
	return {
		...
		done: (value?: any) => doneFn?.(value),
	};
}

test("/context 注册命令，TUI 下渲染标题/分区行/占用条", async () => {
	const harness = contextDialogHarness();
	const run = harness.run();
	assert.equal(harness.registered?.name, "context");
	// 首次 custom 调用即主弹框。
	...
	const component = harness.factory(tui, theme, null, harness.done);
	const lines = stripAnsi(component.render(64).join("\n"));
	assert.match(lines, /Context Usage/);
	assert.match(lines, /240k|30\.0k|\/ 272\.0k tokens \(11\.0%\)/);
	assert.match(lines, /System prompt/);
	assert.match(lines, /Memory/);
	assert.match(lines, /Skills/);
	assert.match(lines, /Tools definition/);
	assert.match(lines, /Tool results/);
	assert.match(lines, /Context/);
	assert.match(lines, /Other/);
	assert.match(lines, /Free space/);
	assert.match(lines, /█+/);
	harness.done(undefined);
	await run;
});

test("/context 非 TUI 回退 notify 文本", async () => {
	...
	assert.match(notified[0]!, /System prompt: 0 tokens/);
	assert.match(notified[0]!, /Free space: \d+(\.\d+)?k? tokens/);
});