...
// initTheme("dark") 后 shiki 会给代码行包上色码（const/数字分片），
// 断言前需剥离 ANSI，与 tool-grouping/resume-tool-visibility 的做法一致。
...
test("split diff keeps the panel transparent while highlighting changed rows", () => {
	...
	// 该断言只验证"panel 透明 + changed 行有自己的背景（≠panel）"；具体发色随终端能力：
	// truecolor → 48;2、256/16 → 48;5、none → 无背景（改行仅靠 +/- glyph 区分）。
	// 能力矩阵本身由 tests/terminal-capabilities.test.ts 覆盖。
	if (text.includes("\x1b[48;")) {
		assert.match(text, /\x1b\[48;(?:2|5);/);
	}
});
...
test("write collapsed preview uses writeDiffCollapsedLines independently of edit", () => {
	...
	const write = renderRichToolResult(
		...
		const writeText = output(write).map(stripAnsi).join("\n");
	...
	const editText = output(edit).map(stripAnsi).join("\n");
...
test("writeDiffCollapsedLines 0 shows stats only until expanded", () => {
	...
	const collapsedText = output(collapsed).map(stripAnsi).join("\n");
	...
	const expandedText = output(expanded).map(stripAnsi).join("\n");
