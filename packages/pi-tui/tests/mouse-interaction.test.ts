...
test("show-more hover targets the view rendered in the current frame after compact", () => {
	...
	const tool = {
		...
		expanded: true,
		state: { tuiStyleIoView: staleView },
		setExpanded(value: boolean) {
			this.expanded = value;
		},
		invalidate() {},
...
test("pi-tui mode off restores native mouse input: no hover/click, wheel still scrolls", async () => {
	...
	try {
		...
		const command = commands.get("tui-style");
		// /tui-style writes the user's real config; back it up and restore it.
		const configPath = join(homedir(), ".pi", "agent", "pi-tui.json");
		...
		try {
			...
			// /tui-style off: hover/click go native, motion reporting stops.
