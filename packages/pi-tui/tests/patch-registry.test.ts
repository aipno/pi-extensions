...
test("singleton storage is globalThis", () => {
	const key = Symbol.for("pi.tui.test.patch-registry");
	try {
		patchRegistry.install(key, { tag: "via-registry" });
		assert.equal((globalThis as unknown as Record<PropertyKey, { tag: string }>)[key].tag, "via-registry");
		(globalThis as unknown as Record<PropertyKey, unknown>)[key] = { tag: "via-globalThis" };
