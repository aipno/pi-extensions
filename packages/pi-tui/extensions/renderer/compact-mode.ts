 ...
 * 最终 agent 回合摘要由 extension 层 feature/agent-summary 独占（agent_end 时
 * appendEntry 一条 markdown 引用块），摘要渲染以 compact-mode 自身逻辑为准。
 *
...
/**
 * 摘要行状态前缀检测（渲染时按当前语言取词）：
 * "Running..."（运行中）与 "Ran for "（已完成）决定配色/动画分支；
 * en 硬编码前缀兜底，兼容 zh 切换前的残留文本与测试用例。
 */
function runningSummaryPrefixes(): string[] {
	return [...new Set([t("summary.running", "Running..."), "Running..."])];
}

function ranForSummaryPrefixes(): string[] {
	const localized = t("summary.ranFor", "Ran for {duration}", { duration: "" }).trim();
	return [...new Set([localized || "Ran for ", "Ran for "])];
}
...
/** assistant stopReason → 外层状态文案（与 Pi 原生口径对齐；fallback 文案走 i18n）。 */
export function messageStopStatus(message: any): string | undefined {
	...
	if (reason === "aborted") {
		return message.errorMessage && message.errorMessage !== "Request was aborted"
			...
			: t("thinking.stop.aborted", "Operation aborted");
	}
	if (reason === "error") {
		const errorMessage = message.errorMessage || t("thinking.stop.unknownError", "Unknown error");
		return t("thinking.stop.errorPrefix", "Error: {message}", { message: errorMessage });
	}
	if (reason === "length") {
		return t("thinking.stop.truncated", "Response was truncated before completion.");
	}
	return undefined;
}
...
function buildMessagesSummary(
	...
	if (runningActive) {
		parts.push(
			durationMs > 0
				? `${t("summary.running", "Running...")} · ${formatThoughtDuration(durationMs)}`
				: t("summary.running", "Running..."),
		);
	} else if (durationMs > 0) {
		parts.push(
			t("summary.ranFor", "Ran for {duration}", { duration: formatThoughtDuration(durationMs) }),
		);
	}
...
function compactAssistantLineComponent(
	...
	return {
		render(width: number): string[] {
			...
			const runningActive = runningSummaryPrefixes().some((prefix) => resolved.startsWith(prefix));
			const plainText = truncateToWidth(resolved, summaryWidth, "…");
			let text = theme.fg("muted", plainText);
			if (runningActive || ranForSummaryPrefixes().some((prefix) => plainText.startsWith(prefix))) {
...
export function installCompactMode(deps: CompactModeInstallDeps): CompactModeHooks {
	...
	return {
		sync(ctx: any) {
			...
			// 与 tracked 集合，这样 /tui-style 切回 compact 时不必 /reload。
			patch.assertAssistantOwnership();
