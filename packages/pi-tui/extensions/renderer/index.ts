...
function applyStyleMode(mode: CompactStyleMode, ctx: any, toolGrouping?: ToolGroupingHooks): void {
	...
	ctx.ui.notify(t("command.tui.modeMessage", "Pi TUI style: {mode}", { mode }), "info");
}
...
export default function (
	...
	pi.registerCommand("tui-style", {
		description: t("command.tui.desc", "Configure Pi TUI style and rich diff options"),
				...
				const languageValues = [
					{
						value: "language=en",
						label: "language=en",
						description: t("command.tui.language.en", "English UI text"),
					},
					{
						value: "language=zh",
						label: "language=zh",
						description: t("command.tui.language.zh", "中文界面文案"),
					},
				];
				return languageValues.filter((item) => item.value.startsWith(prefix));
			}
			const topLevel = [
				{ value: "on", label: "on", description: t("command.tui.on", "Enable Pi TUI style") },
				{
					value: "compact",
					label: "compact",
					description: t("command.tui.compact", "One summary line per assistant message"),
				},
				{ value: "off", label: "off", description: t("command.tui.off", "Use Pi's native renderer") },
				{ value: "status", label: "status", description: t("command.tui.status", "Show full configuration") },
				{
					value: "panel",
					label: "panel",
					description: t("command.tui.panel", "Open interactive settings panel"),
				},
				{
					value: "language",
					label: "language",
					description: t("command.tui.language", "UI text language: en or zh"),
				},
			];
		...
		handler: async (args, ctx) => {
			...
			if (arg.startsWith("language=")) {
				...
				if (lang !== "en" && lang !== "zh") {
					ctx.ui.notify(
						t("command.tui.languageInvalid", "Invalid language: {value}. Use en or zh.", {
							value: lang,
						}),
						"warning",
					);
				...
				ctx.ui.notify(
					t("command.tui.languageUpdated", "Language set to {lang} (applies immediately)", {
						lang,
					}),
					"info",
				);
			...
			if (!arg || arg === "panel" || arg === "on" || arg === "compact" || arg === "off") {
				if (ctx?.mode !== "tui" || !ctx?.hasUI) {
					ctx.ui?.notify?.(t("command.tui.requiresTui", "/tui-style requires TUI mode"), "warning");
					return;
				}
			...
			if (arg === "status") {
				ctx.ui.notify(
					t("command.tui.statusMessage", "Pi TUI style: {status}", {
			...
			ctx.ui.notify(
				t("command.tui.usage", "Usage: /tui-style [on|compact|off|status|panel|language=en|zh]"),
				"warning",
			);
		},
	});
	...
	pi.on("session_start", async (event, ctx) => {
		...
		ctx.ui.setStatus("tui-style", undefined);
