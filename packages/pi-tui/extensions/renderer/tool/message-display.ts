...
/**
 * 接管三个消息组件（`<skill>` 块、压缩摘要、分支摘要），pi-tui on 时渲染为
...
function renderCcstyle(component: any, kind: DisplayKind): void {
	...
	if (component.paddingY !== 0) {
		component._tuiStyleOriginalPaddingY = component.paddingY;
...
export function installMessageDisplayRendering(): () => void {
	...
	const installOne = (ComponentClass: any, kind: DisplayKind): void => {
		...
		const installed = function (this: any) {
			...
			if (this._tuiStyleOriginalPaddingY !== undefined) {
				this.paddingY = this._tuiStyleOriginalPaddingY;
				delete this._tuiStyleOriginalPaddingY;
			}
			original.call(this);
		};
...
/** 遍历当前 transcript，让已挂载的消息组件按当前 mode 重渲染（/tui-style on|off 切换）。 */
