...
function clearAnimation(context: any) {
	if (!context?.state?.tuiStyleAnimationScheduled) return;
	context.state.tuiStyleAnimationScheduled = false;
...
export function clearAllAnimations() {
	for (const ctx of activeAnimationContexts) {
		ctx.state.tuiStyleAnimationScheduled = false;
	}
...
export function scheduleAnimation(context: any, intervalMs = TOOL_LOADING_INTERVAL_MS) {
	...
	if (state.tuiStyleAnimationScheduled) return;
	state.tuiStyleAnimationScheduled = true;
...
export function setToolVisualState(context: any, visualState: ToolVisualState) {
	...
	if (state.tuiStyleToolVisualState === visualState) return;
	state.tuiStyleToolVisualState = visualState;
...
function getToolVisualState(context: any): ToolVisualState | undefined {
	return context?.state?.tuiStyleToolVisualState as ToolVisualState | undefined;
}
...
export function isToolExpanded(options: any, context: any): boolean {
	const local = context?.state?.tuiStyleToolExpanded;
...
const EXPANDED_TOOL_IO_VIEW_GENERATION = Symbol("pi.tui.expanded-tool-io-view");
...
function rememberIoView(context: any, view: ExpandedToolIoView): void {
	...
	context.state.tuiStyleIoView = view;
}
