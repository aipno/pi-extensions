...
type Patch = {
	...
	lastEnabled: boolean;
	theme?: any;
	/** 共享 ticker 的注销句柄；仅在有 pending 组时活跃（空表自动停）。 */
	stopAnimationWatch: (() => void) | null;
};
...
function scheduleGroupAnimation(patch: Patch): void {
	if (patch.stopAnimationWatch || !patch.active) return;
	// 单个共享 80ms ticker 驱动全部活动组（omp issue #8731：N 块并行不再 N 倍唤醒）。
	patch.stopAnimationWatch = getSharedTicker(TOOL_LOADING_INTERVAL_MS).watch(() => {
		if (!patch.active) {
			patch.stopAnimationWatch?.();
			patch.stopAnimationWatch = null;
			return;
		}
...
export class ToolGroupComponent extends Container {
	readonly toolCallId = `pi-tui-tool-group-${nextGroupId++}`;
	...
	render(width: number): string[] {
		...
		if (this._expanded) {
			// 展开面板统一用 user message 背景色（pi-tui 约定），不按状态区分。
...
export function installToolGrouping(getEnabled: () => boolean): ToolGroupingHooks {
	...
	if (previous) {
		...
		previous.stopAnimationWatch?.();
		previous.stopAnimationWatch = null;
		ungroup(previous);
	}
	const original = {
		addChild:
