...
import { renderPartialEditDiff, renderPartialWriteDiff } from "./tool/diff/diff-partial.ts";
...
// pi-subagents 等扩展为 Agent 提供专用渲染器，pi-tui 必须保留。
const DEDICATED_RENDERER_TOOLS = new Set(["Agent"]);

/** edit/write partial：渐进 diff 预览接管 Pending 占位（omp 流式抖动抑制思路）。 */
function renderPendingToolResult(
	toolName: string,
	result: any,
	options: any,
	theme: any,
	...
	const previewOptions = {
		expanded,
		filePath,
		isHovered: () => isToolCallHovered(context?.toolCallId),
	};
	const displayConfig = getToolDisplayConfig;
	if (toolName === "edit") {
		// 宿主 partial updateResult 携带半截 details.diff（omp 抖动的源头）。
		const preview = renderPartialEditDiff(result?.details, previewOptions, displayConfig, theme);
		if (preview) return preview;
	} else if (toolName === "write") {
		// 流式 args 期间即有内容；argsComplete 后走正常 result 渲染路径。
		const content =
			typeof context?.args?.content === "string" && context?.argsComplete !== true
				? context.args.content
				: undefined;
		const preview = renderPartialWriteDiff(content, previewOptions, displayConfig, theme);
		if (preview) return preview;
	}
	...
	return {
		...
		renderResult(result: any, options: any, theme: any, context: any) {
			...
			if (options?.isPartial) {
				return renderPendingToolResult(toolName, result, options, theme, context);
			}
			...
			if (context?.state) context.state.tuiStyleIoView = undefined;
			let cachedWidth: number | undefined;
			let cachedLine: string | undefined;
