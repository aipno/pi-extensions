...
// 差异：
// 1. 不再依赖上游包 —— 配置解耦，由 pi-tui 经 installCompactThinking
...
// 配置由 pi-tui 管控（installCompactThinking 注入初始值，
...
function getThinkingToggleHint() {
	...
	return keys.length > 0 ? t("thinking.toggleExpand", "{keys} to expand", { keys: keys.join("/") }) : undefined;
}

function thinkingExpandAction(): string | undefined {
	return isToolTuiFullscreen() ? t("thinking.clickToShowMore", "click to show more") : getThinkingToggleHint();
}
...
function hiddenPreviewHint(
	...
	if (hiddenLines > 0) {
		const noun = t(
			hiddenLines === 1 ? "thinking.noun.line" : "thinking.noun.lines",
			hiddenLines === 1 ? "line" : "lines",
		);
		const n = String(hiddenLines);
		return {
			prefix: action
				? t("thinking.moreWithAction", " • ({n} more {noun}, {action})", { n, noun, action })
				: t("thinking.morePlain", " • ({n} more {noun})", { n, noun }),
			action: "",
			suffix: "",
		};
	}
	if (forceExpandHint && isToolTuiFullscreen()) {
		return { prefix: " • ", action: t("thinking.clickToShowMore", "click to show more"), suffix: "" };
	}
	return undefined;
}
	...
	prototype.updateContent = function patchedUpdateContent(
		...
		for (let i = 0; i < message.content.length; i++) {
			...
			if (active && latestSummary) {
				...
				const label = self.hiddenThinkingLabel || t("thinking.loading", "Thinking...");
				heading =
				...
				heading = thinkingStyle(
					durationText
						? t("thinking.thoughtFor", "Thought for {duration}", { duration: durationText })
						: (latestSummary?.title ?? t("thinking.thought", "Thought")),
				);
			}
		...
		if (message.stopReason === "length") {
			const stopText = t(
				"thinking.stop.length",
				"Error: Model stopped because it reached the maximum output token limit. The response may be incomplete.",
			);
			self.contentContainer.addChild(new Spacer(1));
			self.contentContainer.addChild(
				new Text(activeTheme ? activeTheme.fg("error", stopText) : stopText, self.outputPad, 0),
			);
			...
			if (message.stopReason === "aborted") {
				...
				const errorMessage =
					message.errorMessage || t("thinking.stop.unknownError", "Unknown error");
				const errorText = t("thinking.stop.errorPrefix", "Error: {message}", { message: errorMessage });
				self.contentContainer.addChild(new Spacer(1));
				self.contentContainer.addChild(
					new Text(
						activeTheme ? activeTheme.fg("error", errorText) : errorText,
						self.outputPad,
						0,
					),
				);
