...
import { BTW_THINKING_LEVELS, type BtwThinkingLevel } from "./side-thread.ts";
...
/** Per-path write queues so concurrent saves cannot interleave temp files on one path. */
...
/**
 * Validate a raw parsed document and project it onto the known settings shape.
 * Unknown and model fields survive updates but never leak into the projection.
 */
...
/** Split "provider/model-id" on the first slash only; model ids may contain more slashes. */
...
/** Read settings, waiting for any in-flight write on the same path to settle first. */
...
/**
 * Apply a patch and publish atomically (same-directory temp file + rename).
 * Preserves `model` and unknown fields; rejects malformed or invalid documents
 * without rewriting them. A missing file is only created by an update.
 */
export function updateBtwSettings(
	...
	return enqueueMutation(settingsPath, async () => {
		...
		await publishSettings(settingsPath, updated, options.signal);
		return settings;
	});
}
