...
import { sanitizeTerminalText } from "./metadata.ts";
import type { StampSettingsPatch, StampSettingsRuntime, StampSettingsState } from "./settings.ts";
...
export function createStampMenu(runtime: StampSettingsRuntime) {
	...
	return defineMenu<StampSettingsState, StampScreen, StampAction>({
		start: "main",
		screens: {
			main: ({ state }) => ({
				kind: "actions",
