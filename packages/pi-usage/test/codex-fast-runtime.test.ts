...
import { correctCodexFastMessageCost } from "../src/codex-fast.ts";
import { registerCodexFastMode } from "../src/codex-fast-runtime.ts";
...
function memoryRuntime(
	...
	const runtime: UsageSettingsRuntime = {
		...
		async update(patch: UsageSettingsPatch, signal) {
...
test("session replacement aborts stale loads and accepted writes before UI publication", async () => {
	...
	releaseLoad({
		kind: "loaded",
		path: "/tmp/pi-usage.json",
		settings: { codexFastMode: true, xaiUsage: false, usageStatusline: true },
