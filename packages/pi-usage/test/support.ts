...
import type { UsageSettingsPatch } from "../src/settings.ts";

/**
 * Apply a settings patch the same way saveUsageSettingsPatch does: empty-string
 * values for the new-api secret/ID keys clear them, whitespace-only values are
 * skipped, and other keys overwrite.
 */
export function applyUsageSettingsPatch(
