...
import {
	...
	queryProviderUsage,
	resolveUsageAuth,
	type ResolvedUsageAuthOptions,
} from "./query.ts";
import { createUsageSettingsRuntime, type UsageSettingsRuntime } from "./settings.ts";
import type {
	PiModel,
	ProviderUsageState,
	ResolvedUsageAuth,
...
import { showUsageSettings } from "./usage-settings-ui.ts";
...
export default function usageExtension(
	...
	const usageAuthOptions = (): ResolvedUsageAuthOptions => {
		const state = settingsRuntime.get();
	...
	const queryAdapterState = async (
		...
		try {
			auth = await awaitWithDeadline(
				resolveUsageAuth(ctx, adapter, undefined, credentialReader, credentialCandidates, usageAuthOptions()),
				signal,
				DEFAULT_TIMEOUT_MS,
					...
					settings: async () => {
						await showUsageSettings(
							...
							(id, _previous, next) => {
								...
								// Credentials changed: drop cached new-api reports and
								// failure backoff so the next query re-resolves auth.
