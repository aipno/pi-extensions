...
/**
 * Per-session live state. Keyed by session id so a detached/child session
 ...
 * (keeps the key a plain string for callers and the sid-gate).
 */
 ...
 * against, and the slot `getRenderState()` resolves to.
 */
...
/**
 * Test-setup reset. Clears BOTH the session Map and the render pointer so
