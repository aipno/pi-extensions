 ...
 *
 * The truncated tail is split into pending / in_progress counts (N1) so the
 * summary line never mislabels an active task as pending.
 */
export interface OverlayLayout {
	visible: readonly Task[];
	hiddenCompleted: number;
	truncatedPending: number;
...
export function selectOverlayLayout(state: TaskState, budget: number): OverlayLayout {
	...
	if (all.length <= budget) {
		return { visible: all, hiddenCompleted: 0, truncatedPending: 0, truncatedInProgress: 0 };
	}
	...
	if (nonCompleted.length <= innerBudget) {
		...
		// Prefer the latest completed rows (end of the list) over the earliest.
	...
	return {
		...
		truncatedPending: truncated.filter((t) => t.status === "pending").length,
