 ...
 *
 * Public re-exports below preserve the import surface so that `index.ts`,
 * `todo-overlay.ts`, and the tests continue to import from `./todo.ts`.
 */
...
// ---------------------------------------------------------------------------
// Public re-exports — other modules and tests keep importing from `./todo.ts`.
// ---------------------------------------------------------------------------
...
export const DEFAULT_PROMPT_GUIDELINES: string[] = [
	...
	"When starting a task from the todo list, mark it in_progress BEFORE beginning work. Mark it completed IMMEDIATELY when done — never batch completions. Exactly one task in_progress at a time; starting a new in_progress task auto-demotes the previous one.",
	...
	"Do not call list or get after mutations — the tool result includes a compact board, and the current list is injected into the system prompt each turn. list is only for deleted or status-filtered views.",
...
export function registerTodoTool(pi: ExtensionAPI): void {
	...
	pi.registerTool({
		...
		label: TOOL_LABEL,
		description:
			"Track multi-step work. Actions: create, update, list, get, delete, clear. Status: pending → in_progress → completed (deleted is a tombstone).",
