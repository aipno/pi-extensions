...
/**
 * Maximum number of option rows visible in the WrappingSelect window. Lifted here so
 * the cap travels with the option-list owner.
 */
...
/**
 * Per-tick projection of OptionListView state. `inputBuffer` is part of the props
 * bag — the session-owned `inlineInput` (a headless `pi-tui` Editor instance)
 * supplies its current `getText()` here per tick. `OptionListView` is purely
 * props-driven; the imperative buffer surface and read-back getters are gone.
 */
