 ...
 * - Both rows render in normal style at all times — partial submission is allowed,
 *   so Submit is never dimmed or visually marked as unselectable. The warning header in
 *   the submit container is the sole signal of incompleteness.
 * - `naturalHeight(width)` is state-INDEPENDENT and returns a constant 2, so the
 *   chrome can subtract a fixed 2 lines without re-rendering.
 */
