...
// ----- Re-exports for test imports — keep `./preview-pane.ts` as the public surface -----
export {
...
/**
 * Per-tick projection of PreviewPane state. The pane reads `selectedIndex` and
 * `focused` from its own props — both derived from canonical state via
 * `selectPreviewPaneProps`. `OptionListView` and `PreviewPane` see the same
 * source of truth without the cross-component live read.
 */
