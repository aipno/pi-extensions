...
import { DEFAULT_COLLAPSE_KEY, formatKeySpecForDisplay } from "../config.ts";
 ...
 * shortcut is configurable (`collapseKey`). Locale entries carry the same
 * `{key}` placeholder and call sites `.replace()` it with
 * `formatKeySpecForDisplay(collapseKey)` after lookup, so per-locale word
 * order is preserved.
 */
