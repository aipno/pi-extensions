 ...
 * resolves `undefined` without rendering anything. `ui.select()` and
 * `ui.input()` ARE functional there — the host renders them natively — so
...
/**
 * Canonical-English fallbacks; resolved through `t()` at dialog time so a live
 * locale applies (top-level `const x = t(...)` would bake load-time English
 * in — see i18n-bridge.ts). The sentinel row label comes from
 ...
 * Structural on purpose: pinned peer types predate `ctx.mode` in some hosts,
