/**
 * pi-ask-user-question — registers the `ask_user_question` tool: a structured
 * option selector with an automatically appended `Type something.` custom-answer
 * row.
 *
 * Sentinel labels and TUI chrome strings localize at render time via the i18n
 * bridge (`state/i18n-bridge.ts`). Strings are keyed in `locales/en.json` (the
 * canonical English key set) and `t(key, fallback)` returns the inline English
 * literal at every call site — swap in a live locale provider later without
 * touching call sites.
 */
...
// none of the session view/TUI render graph that questionnaire-session lazy-loads.
 ...
 * Writes to stdout: the `isTTY` gate both proves an interactive terminal owns the
 * coming wait and keeps the byte out of piped RPC transports (VS Code pendant, Zed).
 */
...
/** Delay before the background session-graph pre-warm. */
export const PREWARM_DELAY_MS = 2000;

type SessionModule = typeof import("./state/questionnaire-session.ts");

type SessionRef = { current: import("./state/questionnaire-session.ts").QuestionnaireSession | null };
...
/**
 * Lazy-load the QuestionnaireSession view/TUI render graph, guarding the two
 * failure shapes of a module graph replaced on disk mid-session. Pi's jiti
 * loader registers a module in its graph cache BEFORE evaluating the body and
 * does not evict it when evaluation throws (jiti 2.7.0), so one failed load —
 * e.g. `npm install --force` replacing the store entry mid-session — leaves
 * every later import of this specifier resolving to a namespace without the
 * class. That state is unrecoverable in-process (cache-busting specifiers fail
 * jiti resolution); both branches therefore return an LLM-facing envelope that
 * names the restart requirement instead of leaking a bare "not a constructor"
 * TypeError.
 */
...
function makeSessionFactory(config: {
	...
	return (
		tui: TUI,
		theme: Theme,
		keybindings: import("./state/questionnaire-session.ts").QuestionnaireSessionConfig["keybindings"],
		done: (result: QuestionnaireResult) => void,
	): import("./state/questionnaire-session.ts").QuestionnaireSessionComponent => {
			...
			editInput: async (value) => {
				try {
					const [{ SettingsManager }, { editWithExternalEditor }] = await Promise.all([
						...
						import("./state/external-editor.ts"),
					]);
...
/**
 * Pre-warm the lazy session graph once startup settles. A graph evaluated while the
 * paths Pi resolved at boot still exist stays in memory for the process lifetime, so
 * later on-disk dependency churn (e.g. `npm install --force` replacing the store
 * mid-session) can no longer poison jiti's graph cache. Swallowed failure is safe:
 * the first real call re-imports and surfaces it through loadQuestionnaireSession's
 * structured envelope. unref keeps the timer from holding a non-TUI embedder's process
 * open.
 */
...
export function registerAskUserQuestionTool(pi: ExtensionAPI): void {
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			// RPC hosts (VSCode pendant, ACP clients like Zed/Paseo): ui.custom()
			// cannot render there, but the select/input dialog sub-protocol works.
			// Hosts that advertise ctx.mode (pi ≥0.79) route to the sequential
			// dialog walker up front, skipping the TUI render-graph import entirely;
			// RPC builds that predate ctx.mode are caught by the custom()-resolved-
			// undefined backstop below. See ./rpc-fallback.ts.
			...
			// Lazy — QuestionnaireSession pulls the view/TUI render graph; load it
			// only when the tool runs, not at extension registration.
