...
// branch would be shadowed by the collision and no key could confirm. With pi
// defaults both names resolve to `enter`, so matching either is equivalent.
...
function routeSubmitTab(
	...
	if (isConfirm(kb, data)) {
		// Submit always submits; Cancel always cancels. The warning header
	...
	// Global note: `n` on the Submit tab opens the notes editor scoped to the whole
...
export function routeKey(data: string, state: QuestionnaireState, runtime: QuestionnaireRuntime): QuestionnaireAction {
	...
	// see no visible modal and chat scroll resumes. The toggle key is also captured at
	// the raw terminal level via `ctx.ui.onTerminalInput` so it still routes here when
	// the overlay is hidden (pi-tui does not deliver input to a hidden overlay's
	...
	// importing COLLAPSE_KEY_OFF would pull ../config.ts (and its config
	...
	// Universal `n` activation: the notes editor opens on every question tab
