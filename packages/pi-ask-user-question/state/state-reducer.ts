...
export type Effect =
	 ...
	 * which lets other overlay-aware consumers see the questionnaire
...
/**
 * Pure reducer: (state, action, ctx) → (state, Effect[]). Delegates to `HANDLERS` —
 * per-kind handlers above are pure, named, and individually testable.
